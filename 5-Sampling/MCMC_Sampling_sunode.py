...
from run_sampling import run_model_sunode
...
# TODO: MANUAL - Set priors really tight around true values for all params (except sigma)
use_tight_priors = False
if use_tight_priors:
    for param in priors.keys():
        if param in true_values:
            # Set mean to true value
            priors[param][0] = true_values[param]
            # Set std deviation to a very small value
            priors[param][1] = 0.01
...
model_t_eval = Data_Influent_states['Time'].to_numpy()
...
# Ode system
model_ode_system = lambda t, y, theta: ode_system_sunode(t, y, theta, model_input_data, model_reactor_volumes, use_polynomial=True)
...
scaler_mean = pca_results['scaler_mean']
scaler_scale = pca_results['scaler_scale']
# pca_param_names = ['K_X', 'k_STO', 'K_S', 'mu_H',..] as example
pca_param_names = pca_results['param_names']
n_params_pca = len(pca_param_names)
...
eigenvectors_transposed = pt.constant(eigenvectors.T)
mean_pt = pt.constant(scaler_mean)
scale_pt = pt.constant(scaler_scale)
...
with pymc.Model() as model:
    ...
    # # Alt: Include all params in model
    #  ## AllFree Code
    ...
    # eta_NOX = pymc.TruncatedNormal("eta_NOX", mu=priors['eta_NOX'][0], sigma=priors['eta_NOX'][1], initval=priors['eta_NOX'][0], lower=0)
    # K_O2 =    pymc.TruncatedNormal("K_O2", mu=priors['K_O2'][0], sigma=priors['K_O2'][1], initval=priors['K_O2'][0], lower=0)
    # K_NOX =   pymc.TruncatedNormal("K_NOX", mu=priors['K_NOX'][0], sigma=priors['K_NOX'][1], initval=priors['K_NOX'][0], lower=0)
    # K_S =     pymc.TruncatedNormal("K_S", mu=priors['K_S'][0], sigma=priors['K_S'][1], initval=priors['K_S'][0], lower=0)
    # K_STO =   pymc.TruncatedNormal("K_STO", mu=priors['K_STO'][0], sigma=priors['K_STO'][1], initval=priors['K_STO'][0], lower=0)
    # mu_H =    pymc.TruncatedNormal("mu_H", mu=priors['mu_H'][0], sigma=priors['mu_H'][1], initval=priors['mu_H'][0], lower=0)
    # K_NH4 =   pymc.TruncatedNormal("K_NH4", mu=priors['K_NH4'][0], sigma=priors['K_NH4'][1], initval=priors['K_NH4'][0], lower=0)
    # K_ALK =   pymc.TruncatedNormal("K_ALK", mu=priors['K_ALK'][0], sigma=priors['K_ALK'][1], initval=priors['K_ALK'][0], lower=0)
    # b_H_O2 =  pymc.TruncatedNormal("b_H_O2", mu=priors['b_H_O2'][0], sigma=priors['b_H_O2'][1], initval=priors['b_H_O2'][0], lower=0)
    # b_H_NOX = pymc.TruncatedNormal("b_H_NOX", mu=priors['b_H_NOX'][0], sigma=priors['b_H_NOX'][1], initval=priors['b_H_NOX'][0], lower=0)
    # b_STO_O2 = pymc.TruncatedNormal("b_STO_O2", mu=priors['b_STO_O2'][0], sigma=priors['b_STO_O2'][1], initval=priors['b_STO_O2'][0], lower=0)
    # b_STO_NOX = pymc.TruncatedNormal("b_STO_NOX", mu=priors['b_STO_NOX'][0], sigma=priors['b_STO_NOX'][1], initval=priors['b_STO_NOX'][0], lower=0)
    ...
    # K_A_O2 =  pymc.TruncatedNormal("K_A_O2", mu=priors['K_A_O2'][0], sigma=priors['K_A_O2'][1], initval=priors['K_A_O2'][0], lower=0)
    # K_A_ALK = pymc.TruncatedNormal("K_A_ALK", mu=priors['K_A_ALK'][0], sigma=priors['K_A_ALK'][1], initval=priors['K_A_ALK'][0], lower=0)
    # b_A_O2 =  pymc.TruncatedNormal("b_A_O2", mu=priors['b_A_O2'][0], sigma=priors['b_A_O2'][1], initval=priors['b_A_O2'][0], lower=0)
    # b_A_NOX = pymc.TruncatedNormal("b_A_NOX", mu=priors['b_A_NOX'][0], sigma=priors['b_A_NOX'][1], initval=priors['b_A_NOX'][0], lower=0)
    # f_S_I =   pymc.TruncatedNormal("f_S_I", mu=priors['f_S_I'][0], sigma=priors['f_S_I'][1], initval=priors['f_S_I'][0], lower=0)
    ...
    # f_X_I =   pymc.TruncatedNormal("f_X_I", mu=priors['f_X_I'][0], sigma=priors['f_X_I'][1], initval=priors['f_X_I'][0], lower=0)
    # i_N_S_I = pymc.TruncatedNormal("i_N_S_I", mu=priors['i_N_S_I'][0], sigma=priors['i_N_S_I'][1], initval=priors['i_N_S_I'][0], lower=0)
    # i_N_S_S = pymc.TruncatedNormal("i_N_S_S", mu=priors['i_N_S_S'][0], sigma=priors['i_N_S_S'][1], initval=priors['i_N_S_S'][0], lower=0)
    # i_N_X_I = pymc.TruncatedNormal("i_N_X_I", mu=priors['i_N_X_I'][0], sigma=priors['i_N_X_I'][1], initval=priors['i_N_X_I'][0], lower=0)
    # i_N_X_S = pymc.TruncatedNormal("i_N_X_S", mu=priors['i_N_X_S'][0], sigma=priors['i_N_X_S'][1], initval=priors['i_N_X_S'][0], lower=0)
    # i_N_BM =  pymc.TruncatedNormal("i_N_BM", mu=priors['i_N_BM'][0], sigma=priors['i_N_BM'][1], initval=priors['i_N_BM'][0], lower=0)
    # # i_SS_X_I = pymc.TruncatedNormal("i_SS_X_I", mu=priors['i_SS_X_I'][0], sigma=priors['i_SS_X_I'][1], initval=priors['i_SS_X_I'][0], lower=0)
    # # i_SS_X_S = pymc.TruncatedNormal("i_SS_X_S", mu=priors['i_SS_X_S'][0], sigma=priors['i_SS_X_S'][1], initval=priors['i_SS_X_S'][0], lower=0)
    # # i_SS_BM =  pymc.TruncatedNormal("i_SS_BM", mu=priors['i_SS_BM'][0], sigma=priors['i_SS_BM'][1], initval=priors['i_SS_BM'][0], lower=0)

    # # Alt - Uninformed from 0 to 30 for all parameters
    # k_H = pymc.Uniform("k_H", lower=0, upper=30, initval=priors['k_H'][0])
    # K_X = pymc.Uniform("K_X", lower=0, upper=30, initval=priors['K_X'][0])
    # k_STO = pymc.Uniform("k_STO", lower=0, upper=30, initval=priors['k_STO'][0])
    # eta_NOX = pymc.Uniform("eta_NOX", lower=0, upper=30, initval=priors['eta_NOX'][0])
    # K_O2 = pymc.Uniform("K_O2", lower=0, upper=30, initval=priors['K_O2'][0])
    # K_NOX = pymc.Uniform("K_NOX", lower=0, upper=30, initval=priors['K_NOX'][0])
    # K_S = pymc.Uniform("K_S", lower=0, upper=30, initval=priors['K_S'][0])
    # K_STO = pymc.Uniform("K_STO", lower=0, upper=30, initval=priors['K_STO'][0])
    # mu_H = pymc.Uniform("mu_H", lower=0, upper=30, initval=priors['mu_H'][0])
    # K_NH4 = pymc.Uniform("K_NH4", lower=0, upper=30, initval=priors['K_NH4'][0])
    # K_ALK = pymc.Uniform("K_ALK", lower=0, upper=30, initval=priors['K_ALK'][0])
    # b_H_O2 = pymc.Uniform("b_H_O2", lower=0, upper=30, initval=priors['b_H_O2'][0])
    # b_H_NOX = pymc.Uniform("b_H_NOX", lower=0, upper=30, initval=priors['b_H_NOX'][0])
    # b_STO_O2 = pymc.Uniform("b_STO_O2", lower=0, upper=30, initval=priors['b_STO_O2'][0])
    # b_STO_NOX = pymc.Uniform("b_STO_NOX", lower=0, upper=30, initval=priors['b_STO_NOX'][0])
    # mu_A = pymc.Uniform("mu_A", lower=0, upper=30, initval=priors['mu_A'][0])
    # K_A_NH4 = pymc.Uniform("K_A_NH4", lower=0, upper=30, initval=priors['K_A_NH4'][0])
    # K_A_O2 = pymc.Uniform("K_A_O2", lower=0, upper=30, initval=priors['K_A_O2'][0])
    # K_A_ALK = pymc.Uniform("K_A_ALK", lower=0, upper=30, initval=priors['K_A_ALK'][0])
    # b_A_O2 = pymc.Uniform("b_A_O2", lower=0, upper=30, initval=priors['b_A_O2'][0])
    # b_A_NOX = pymc.Uniform("b_A_NOX", lower=0, upper=30, initval=priors['b_A_NOX'][0])
    # f_S_I = pymc.Uniform("f_S_I", lower=0, upper=30, initval=priors['f_S_I'][0])
    # Y_STO_O2 = pymc.Uniform("Y_STO_O2", lower=0, upper=30, initval=priors['Y_STO_O2'][0])
    # Y_STO_NOX = pymc.Uniform("Y_STO_NOX", lower=0, upper=30, initval=priors['Y_STO_NOX'][0])
    # Y_H_O2 = pymc.Uniform("Y_H_O2", lower=0, upper=30, initval=priors['Y_H_O2'][0])
    # Y_H_NOX = pymc.Uniform("Y_H_NOX", lower=0, upper=30, initval=priors['Y_H_NOX'][0])
    # Y_A = pymc.Uniform("Y_A", lower=0, upper=30, initval=priors['Y_A'][0])
    # f_X_I = pymc.Uniform("f_X_I", lower=0, upper=30, initval=priors['f_X_I'][0])
    # i_N_S_I = pymc.Uniform("i_N_S_I", lower=0, upper=30, initval=priors['i_N_S_I'][0])
    # i_N_S_S = pymc.Uniform("i_N_S_S", lower=0, upper=30, initval=priors['i_N_S_S'][0])
    # i_N_X_I = pymc.Uniform("i_N_X_I", lower=0, upper=30, initval=priors['i_N_X_I'][0])
    # i_N_X_S = pymc.Uniform("i_N_X_S", lower=0, upper=30, initval=priors['i_N_X_S'][0])
    # i_N_BM = pymc.Uniform("i_N_BM", lower=0, upper=30, initval=priors['i_N_BM'][0])
    #     # Always Fixed
    ...
    # The following parameters are fixed to prior means:
        # i_SS_BM, i_SS_X_I, i_SS_X_S, f_S_I, K_NH4, i_N_S_I, i_N_X_I, i_N_S_S, K_ALK, K_STO, b_A_NOX, b_STO_NOX, i_N_X_S, b_H_NOX, f_X_I, b_A_O2, i_N_BM, K_O2, b_STO_O2, b_H_O2
    # The following parameters are identifiable from the PLA graphs:
        #  k_H, K_X, mu_H, Y_STO_O2, Y_STO_NOX, Y_H_O2, 
    # The others are non-identifiable: 
        # k_STO, eta_NOX, K_NOX, K_S, mu_A, K_A_NH4, K_A_O2, K_A_ALK, Y_H_NOX, Y_A, 
    ...
    K_S = pymc.Deterministic("K_S", theta_pca[pca_param_names.index("K_S")])
    mu_A = pymc.Deterministic("mu_A", theta_pca[pca_param_names.index("mu_A")])
    K_A_NH4 = pymc.Deterministic("K_A_NH4", theta_pca[pca_param_names.index("K_A_NH4")])
    K_A_O2 = pymc.Deterministic("K_A_O2", theta_pca[pca_param_names.index("K_A_O2")])
    K_A_ALK = pymc.Deterministic("K_A_ALK", theta_pca[pca_param_names.index("K_A_ALK")])
    Y_H_NOX = pymc.Deterministic("Y_H_NOX", theta_pca[pca_param_names.index("Y_H_NOX")])
    Y_A = pymc.Deterministic("Y_A", theta_pca[pca_param_names.index("Y_A")])

    # ## Special cases for 0 from PLA
    #     # TODO: MANUAL
    # EXAMPLE: K_A_ALK = 1e-6

    model_theta = {
        # Each parameter of the ode in theta.
        'k_H':      (k_H, ()),
        'K_X':      (K_X, ()),
        'k_STO':    (k_STO, ()),
        'eta_NOX':  (eta_NOX, ()),
        ...
        'i_SS_X_S': (i_SS_X_S, ()),
        'i_SS_BM':  (i_SS_BM, ()),
        # Extra parameters to account for sigma
        'extra':    np.zeros(6),
    }

    # ODE system solution
    y_hat, _, problem, solver, _, _ = sunode_pytensor.solve_ivp(
        y0=model_y0,            # Initial conditions of state variables
        params= model_theta,    # Parameters of the ODE system
        rhs=model_ode_system,   # ODE system function
        tvals=model_t_eval,     # Time points to evaluate the ODE system
        t0=model_t_eval[0],     # Initial time to evaluate the ODE system
        solver_kwargs={'solver': solver_method},  # Solver options
    )

    # Change mxsteps and tolerances for the solver
    # lib = sunode._cvodes.lib
    # tol = 1e-8
    # lib.CVodeSStolerances(solver._ode, tol, tol)
    # lib.CVodeSStolerancesB(solver._ode, solver._odeB, tol, tol)
    # lib.CVodeQuadSStolerancesB(solver._ode, solver._odeB, tol, tol)
...
if __name__ == '__main__':
    __spec__ = None
    # Using NUTS sampler
    # trace = run_model_sunode(model=model, tune=config_tuning_samples, draws=config_draw_samples, chains=config_sample_chains, cores=config_sample_cores,
    #                                 progressbar='combined', init='adapt_diag')
    trace = run_model_sunode(model=model, tune=config_tuning_samples, draws=config_draw_samples, chains=4, cores=1,
                                progressbar='combined', init='adapt_diag')
...
# trace_sunode.to_netcdf(results_dir + "/trace_sunode.nc")
trace.to_netcdf(results_dir / "trace.nc")

# trace_sunode.to_netcdf(resu"trace_sunode.nc")  # Save to current directory for testing
